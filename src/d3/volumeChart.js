...
import {groupTrees, groupTreesVolume} from "../helpers/d3Helper";
...
trunkChart.render = (el, data, version) => {
    ...
    d3.select(el).select('.titleText').text(version);
    ...
    d3.select(el).select('.yAxis').transition().duration(500).call(d3.axisLeft(y).tickFormat(d=> d+" m³"));
